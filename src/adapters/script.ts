...
function termTree(child: ChildProcess): void {
  if (child.pid == null) return
  if (process.platform === 'win32') {
    try { spawn('taskkill', ['/pid', String(child.pid), '/t'], { windowsHide: true }).on('error', () => {}) }
...
export const scriptAdapter: Adapter = {
  async *stream(messages: Message[], config: AdapterConfig): AsyncIterable<Chunk> {
    ...
    const emit = (chunks: Chunk[]) => {
      if (terminal) return
      for (const c of chunks) {
        queue.push(c)
        if (c.type === 'done') sawDone = true
        if (c.type === 'error' && c.scope) terminal = true
        if (c.type === 'step') {
          steps++
          if (c.step.cost != null) costUsd += c.step.cost
        }
      }
      if (chunks.length) wake()
      if (!terminal) enforceLimits()
    }

    let child: ChildProcess
    let killTimer: NodeJS.Timeout | undefined
    let hardTimer: NodeJS.Timeout | undefined
    const stopTimers = () => { if (killTimer) clearTimeout(killTimer); if (hardTimer) clearTimeout(hardTimer) }
    ...
    function enforceLimits(): void {
      if (a.maxSteps != null && steps > a.maxSteps) abort(`step limit reached (${a.maxSteps})`)
      else if (a.maxCostUsd != null && costUsd > a.maxCostUsd) abort(`cost limit reached ($${a.maxCostUsd})`)
    }
    ...
    stdin?.on('error', () => { /* agent may exit before reading stdin */ })
    child.on('error', e => { if (!terminal) { terminal = true; queue.push({ type: 'error', message: e.message, scope: 'agent' }) } stopTimers(); finish() })
    child.on('close', code => {
      stopTimers()
    ...
    try { stdin?.write(JSON.stringify({ messages, model: config.model })) } catch { /* ignore */ }
    try { stdin?.end() } catch { /* ignore */ }

    try {
      for (;;) {
        if (queue.length) { yield queue.shift() as Chunk; continue }
        if (finished) break
