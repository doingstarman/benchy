...
export type ThinkPart = { type: 'token' | 'reasoning'; text: string }
...
export class ThinkTagParser {
  ...
  push(text: string): ThinkPart[] {
    if (!this.armed) return text ? [{ type: 'token', text }] : []
    ...
    for (;;) {
      ...
      if (at !== -1) {
        ...
        // re-open one.
        if (!this.inside) this.armed = false
        continue
      }
  ...
  private emit(out: ThinkPart[], text: string) {
    if (!text) return
    if (this.inside) {
