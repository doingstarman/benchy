...
-- Metrics registry: USER-DEFINED metrics only. Built-ins live in code
-- (src/metrics/builtins.ts) and resolve over existing columns/functions at read time
-- — never stored here. A custom metric is an expression over other metric keys.
CREATE TABLE IF NOT EXISTS metrics (
  ...
  -- JSON TargetKind[]: which participant kinds this metric applies to.
  applies_to TEXT NOT NULL DEFAULT '["model","agent","pipeline"]',
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);

-- Materialized values for CUSTOM metrics only. result_id set for per-answer scope,
-- run_id for per-run. value NULL = "no value produced" (never 0).
...
-- benchy OBSERVES these; it never executes them. step_index preserves emission
-- order; parent_id nests up to 3 levels. payload is capped; cost NULL = unknown.
