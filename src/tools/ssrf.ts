...
//
// The check is on the address bytes, never the string. "localhost",
// "127.0.0.1", "0177.0.0.1" (octal), "2130706433" (integer), "[::1]", the
// IPv4-mapped form in either "::ffff:127.0.0.1" or its compressed-hex
// "::ffff:7f00:1" spelling, and a public hostname whose DNS record points at
// loopback all denote the same forbidden address. Regex-matching one text form
// missed the others — the hex-mapped spelling was a real bypass — so everything
// is decoded to bytes and judged there.

export interface VettedAddress { address: string; family: 4 | 6 }
...
function isBlockedIpv6(ip: string): boolean {
  ...
  if (!b) return false
  if (b.every((x, i) => (i === 15 ? x === 1 : x === 0))) return true // ::1 loopback
  ...
  if (mapped || compat) return isBlockedIpv4Int(((b[12] << 24) >>> 0) + (b[13] << 16) + (b[14] << 8) + b[15])
...
// record is safe.
export async function assertPublicHost(hostname: string): Promise<VettedAddress> {
