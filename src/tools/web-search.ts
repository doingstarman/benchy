...
async function brave(query: string, apiKey: string): Promise<SearchHit[]> {
  const res = await fetch(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=5`, {
    headers: { 'X-Subscription-Token': apiKey, accept: 'application/json' },
    signal: AbortSignal.timeout(15_000),
  })
  if (!res.ok) throw new Error(`Brave search failed with status ${res.status}`)
  const data = await res.json() as { web?: { results?: { title: string; url: string; description: string }[] } }
  return (data.web?.results ?? []).map(r => ({ title: r.title, url: r.url, snippet: r.description }))
}

async function tavily(query: string, apiKey: string): Promise<SearchHit[]> {
...
export function makeWebSearchTool(config: SearchConfig): Tool {
  return {
    spec: {
      name: 'web_search',
