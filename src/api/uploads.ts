...
export function bindAttachmentToDataset(id: string, datasetId: string): void {
...
export async function cloneAttachmentOnto(
  attachmentId: string, targetRunId: string, targetPromptIndex: number,
): Promise<void> {
  ...
  getDb().prepare(
    'INSERT INTO attachments (id, run_id, prompt_index, mime_type, name, size, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(newId, targetRunId, targetPromptIndex, row.mime_type, row.name, row.size, Date.now())
}
...
export async function deleteAttachmentsForDataset(datasetId: string): Promise<void> {
  const db = getDb()
  const rows = db.prepare('SELECT id, mime_type FROM attachments WHERE dataset_id = ?')
    .all(datasetId) as { id: string; mime_type: string }[]
  for (const row of rows) {
    await unlink(uploadPath(row.id, row.mime_type)).catch(() => {})
  }
...
export async function registerUploadsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.delete<{ Params: { id: string } }>('/api/uploads/:id', async (req, reply) => {
    ...
    if (row.dataset_id) return reply.code(409).send({ error: 'Attachment belongs to a dataset — delete the dataset item instead' })
