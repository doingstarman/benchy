...
export const AGGREGATES = ['mean', 'median', 'p50', 'p95', 'min', 'max', 'sum'] as const
export const SCALAR_FUNCTIONS = ['abs', 'round', 'clamp'] as const
export type AggregateName = typeof AGGREGATES[number]

const AGG_SET = new Set<string>(AGGREGATES)
...
export type Node =
  | { t: 'num'; v: number }
  | { t: 'ident'; name: string; span: [number, number] }
  | { t: 'unary'; op: '-'; x: Node }
  | { t: 'binary'; op: '+' | '-' | '*' | '/'; l: Node; r: Node }
  | { t: 'call'; name: string; args: Node[]; span: [number, number] }

export interface ExprError { message: string; span: [number, number]; suggestion?: string }

interface Tok { t: 'num' | 'ident' | 'op' | 'lparen' | 'rparen' | 'comma'; text: string; start: number }
...
function tokenize(src: string): Tok[] {
  ...
  while ((m = re.exec(src))) {
    if (m[0].trim() === '' && !m[1] && !m[2] && !m[3] && !m[4] && !m[5] && !m[6]) continue
    const start = m.index
    if (m[1]) toks.push({ t: 'ident', text: m[1], start })
    else if (m[2]) toks.push({ t: 'num', text: m[2], start })
    else if (m[3]) toks.push({ t: 'op', text: m[3], start })
    else if (m[4]) toks.push({ t: 'lparen', text: '(', start })
    else if (m[5]) toks.push({ t: 'rparen', text: ')', start })
    else if (m[6]) toks.push({ t: 'comma', text: ',', start })
    else if (m[7]) throw new ParseError(`Unexpected character "${m[7]}"`, [start, start + 1])
  }
  return toks
}
...
function parseTokens(toks: Tok[], src: string): Node {
  ...
  function expr(minBp: number): Node {
    ...
    while (pos < toks.length) {
      const t = toks[pos]
      if (t.t !== 'op' || (t.text !== '+' && t.text !== '-' && t.text !== '*' && t.text !== '/')) break
  ...
  function unary(): Node {
    const t = peek()
    if (t && t.t === 'op' && t.text === '-') { pos++; return { t: 'unary', op: '-', x: unary() } }
    if (t && t.t === 'op' && t.text === '+') { pos++; return unary() }
    return atom()
  }
  ...
  function atom(): Node {
    ...
    if (t.t === 'lparen') {
      ...
      const inner = expr(1)
      const close = peek()
      if (!close || close.t !== 'rparen') throw new ParseError('Unbalanced bracket — this group is never closed', [t.start, t.start + 1])
      pos++
      return inner
    }
    if (t.t === 'ident') {
      ...
      if (next && next.t === 'lparen') {
        ...
        if (peek() && peek().t !== 'rparen') {
          args.push(expr(1))
          while (peek() && peek().t === 'comma') { pos++; args.push(expr(1)) }
        }
        const close = peek()
        if (!close || close.t !== 'rparen') throw new ParseError(`Unbalanced bracket — ${t.text}( is never closed`, [next.start, next.start + 1])
        pos++
  ...
  if (toks.length === 0) throw new ParseError('Expression is empty', [0, 0])
  const node = expr(1)
  if (pos < toks.length) { const t = toks[pos]; throw new ParseError(`Unexpected "${t.text}"`, [t.start, t.start + t.text.length]) }
  return node
}

export function parse(src: string): { ast: Node | null; error: ExprError | null } {
  try {
...
function nearestKey(name: string, keys: string[]): string | undefined {
  let best: string | undefined, bestD = Infinity
  for (const k of keys) { const dd = levenshtein(name, k); if (dd < bestD) { bestD = dd; best = k } }
  return bestD <= Math.max(2, Math.floor(name.length / 3)) ? best : undefined
}

function walk(n: Node, fn: (n: Node) => void): void {
  fn(n)
  if (n.t === 'unary') walk(n.x, fn)
  else if (n.t === 'binary') { walk(n.l, fn); walk(n.r, fn) }
  else if (n.t === 'call') n.args.forEach(a => walk(a, fn))
}
...
function identsOutsideAggregates(n: Node, insideAgg: boolean, out: Node[]): void {
  if (n.t === 'ident' && !insideAgg) out.push(n)
  else if (n.t === 'unary') identsOutsideAggregates(n.x, insideAgg, out)
  else if (n.t === 'binary') { identsOutsideAggregates(n.l, insideAgg, out); identsOutsideAggregates(n.r, insideAgg, out) }
  else if (n.t === 'call') { const agg = AGG_SET.has(n.name); n.args.forEach(a => identsOutsideAggregates(a, insideAgg || agg, out)) }
}
...
export function validate(src: string, knownKeys: string[], scope: 'answer' | 'run'): ValidateResult {
  const { ast, error } = parse(src)
  if (!ast) return { ok: false, error: error ?? undefined, refs: [], usesAggregate: false }

  const refs = new Set<string>()
  let usesAggregate = false
  let fail: ExprError | undefined

  walk(ast, n => {
    if (fail) return
    if (n.t === 'ident') {
      if (!knownKeys.includes(n.name)) fail = { message: `No metric named ${n.name}`, span: n.span, suggestion: nearestKey(n.name, knownKeys) }
      else refs.add(n.name)
    } else if (n.t === 'call') {
      const arity = FUNC_ARITY[n.name]
      if (!arity) fail = { message: `Unknown function ${n.name}(`, span: n.span, suggestion: nearestKey(n.name, [...AGGREGATES, ...SCALAR_FUNCTIONS]) }
      else if (n.args.length < arity[0] || n.args.length > arity[1]) fail = { message: `${n.name}() takes ${arity[0] === arity[1] ? arity[0] : `${arity[0]}–${arity[1]}`} argument(s)`, span: n.span }
      else if (AGG_SET.has(n.name)) usesAggregate = true
    }
  })
  if (fail) return { ok: false, error: fail, refs: [...refs], usesAggregate }
  ...
  if (usesAggregate && scope === 'run') {
    ...
    if (bare.length) {
      const b = bare[0] as Extract<Node, { t: 'ident' }>
...
function applyScalar(node: Extract<Node, { t: 'call' }>, args: (number | null)[]): number | null {
...
export function evaluateRun(ast: Node, series: Scope[], scopeAggregate: string): number | null {
  let hasAgg = false
  walk(ast, n => { if (n.t === 'call' && AGG_SET.has(n.name)) hasAgg = true })
  if (!hasAgg) {
    return aggregate(scopeAggregate, series.map(s => evaluate(ast, s)).filter((v): v is number => v != null))
  }
  return evalRunNode(ast, series)
}

function evalRunNode(ast: Node, series: Scope[]): number | null {
  switch (ast.t) {
    case 'num': return ast.v
    ...
    case 'call':
      if (AGG_SET.has(ast.name)) {
        return aggregate(ast.name, series.map(s => evaluate(ast.args[0], s)).filter((v): v is number => v != null))
      }
