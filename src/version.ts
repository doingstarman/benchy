...
// Where this install actually keeps its files — the Settings screen must show
// the real port/paths, not build-time guesses (dev and prod differ).
...
// Why the latest build is unknown — "couldn't reach GitHub" and "GitHub has no
// version.json yet" are different truths and the UI must not confuse them.
export type CheckError = 'network' | 'missing' | null
...
const DEV_VERSION: BuildVersion = { sha: 'dev', commitDate: null, builtAt: null }

let localCache: BuildVersion | undefined
...
async function readLocalVersion(): Promise<BuildVersion> {
  if (localCache) return localCache
  try {
    const path = join(dirname(fileURLToPath(import.meta.url)), 'version.json')
    const parsed = JSON.parse(await readFile(path, 'utf8')) as BuildVersion
    localCache = parsed
  } catch {
...
function toBuildVersion(raw: unknown): BuildVersion | null {
  if (!raw || typeof raw !== 'object') return null
  const v = raw as Record<string, unknown>
  if (typeof v.builtAt !== 'string' || !ISO_UTC.test(v.builtAt)) return null
  return {
...
async function fetchRemote(): Promise<RemoteState> {
  const ttl = remoteCache?.checkError === 'network' ? REMOTE_FAIL_TTL_MS : REMOTE_TTL_MS
  if (remoteCache && Date.now() - remoteCache.fetchedAt < ttl) return remoteCache
  ...
  let latest: BuildVersion | null = null
  let checkError: CheckError = null
  try {
  ...
  try {
    const commits = await get(COMMITS_URL) as { sha?: unknown; commit?: { message?: unknown } }[] | null
    changes = (commits ?? [])
      .filter(c => typeof c?.sha === 'string' && typeof c.commit?.message === 'string')
      .map(c => ({ sha: (c.sha as string).slice(0, 7), message: (c.commit!.message as string).split('\n')[0] }))
...
export function isNewer(latest: BuildVersion | null, current: BuildVersion): boolean {
  ...
  if (typeof latest?.builtAt !== 'string' || !ISO_UTC.test(latest.builtAt)) return false
  if (typeof current.builtAt !== 'string' || !ISO_UTC.test(current.builtAt)) return false
  return latest.builtAt > current.builtAt
}

export type UpdateStatus = Omit<VersionInfo, 'repoUrl' | 'runtime'>
...
export async function getVersionInfo(runtime: RuntimeInfo, force = false): Promise<VersionInfo> {
