...
export function logEvent(level, category, message, meta) {
    try {
        const db = getDb();
...
export function queryLogs(f) {
    ...
    const rows = getDb().prepare(`SELECT id, ts, level, category, message, meta FROM logs ${clause} ORDER BY ts DESC, id DESC LIMIT ?`).all(...args);
...
export function countLogs() {
    return getDb().prepare('SELECT COUNT(*) AS c FROM logs').get().c;
}
