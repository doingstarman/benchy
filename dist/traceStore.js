...
export function insertTraceSteps(resultId, steps) {
    if (steps.length === 0)
        return;
    const db = getDb();
    const now = Date.now();
    const gid = (id) => `${resultId}:${id}`;
    const stmt = db.prepare(`INSERT INTO trace_steps
    ...
    db.transaction(() => {
        steps.forEach((s, i) => {
            stmt.run(gid(s.id), resultId, i, s.parentId != null ? gid(s.parentId) : null, s.kind, s.name, s.ms, s.inputTokens, s.outputTokens, s.cost, s.payload, s.payloadTruncated ? 1 : 0, s.isError ? 1 : 0, now);
        });
    })();
}
...
// suffices).
export function readTrace(resultId) {
    const rows = getDb().prepare(`SELECT id, parent_id, kind, name, ms, input_tokens, output_tokens, cost, payload, payload_truncated, is_error
       FROM trace_steps WHERE result_id = ? ORDER BY step_index`).all(resultId);
    const depthById = new Map();
    return rows.map(r => {
        const depth = r.parent_id != null && depthById.has(r.parent_id) ? depthById.get(r.parent_id) + 1 : 0;
...
export function traceAggregate(resultId) {
    const rows = getDb().prepare('SELECT kind, cost, is_error FROM trace_steps WHERE result_id = ?').all(resultId);
    if (rows.length === 0)
        return null;
