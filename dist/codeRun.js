...
function countTests(language, testSource) {
    const re = language === 'python'
        ? /^(?:async[ \t]+)?def[ \t]+test_\w+/gm
        : /\btest\s*\(\s*[`'"]/g;
    return (testSource.match(re) ?? []).length;
}
function pythonRunner(solution, tests, nonce) {
...
function jsRunner(solution, tests, nonce) {
...
export function interpreterCommand(language) {
    if (language === 'javascript')
        return process.execPath;
    if (interpreterCache.has('python'))
        return interpreterCache.get('python') ?? null;
    let found = null;
...
export async function runTests(language, modelText, testSource, opts = {}) {
    const declared = countTests(language, testSource);
    const fail = (error) => ({ passed: 0, total: declared, cases: [], error });
    const cmd = interpreterCommand(language);
    if (!cmd)
        return fail(`no ${language === 'python' ? 'Python' : 'Node'} interpreter on PATH`);
...
function exec(cmd, file, cwd, timeoutMs) {
...
function parseResult(stdout, nonce) {
    ...
    try {
        ...
        const cases = Array.isArray(o.cases)
            ? o.cases.filter((c) => !!c && typeof c.name === 'string' && typeof c.ok === 'boolean')
            : [];
