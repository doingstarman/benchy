...
export async function registerDashboardRoutes(app) {
    app.get('/api/dashboard', async () => {
        ...
        const kindCounts = db.prepare('SELECT kind, COUNT(*) AS c FROM targets GROUP BY kind').all();
        const kc = (k) => kindCounts.find(x => x.kind === k)?.c ?? 0;
        // Per (provider, model) rollup for cost + the leaderboard.
        const groups = db.prepare(`SELECT provider_id, model, COUNT(*) AS results,
              COALESCE(SUM(input_tokens),0) AS in_tok, COALESCE(SUM(output_tokens),0) AS out_tok,
              AVG(score) AS avg_score
       FROM results GROUP BY provider_id, model`).all();
        const traceRows = db.prepare(`SELECT r.provider_id AS pid, r.model AS model, COALESCE(SUM(ts.cost),0) AS cost
       FROM trace_steps ts JOIN results r ON r.id = ts.result_id
       GROUP BY r.provider_id, r.model`).all();
        const traceCost = new Map(traceRows.map(x => [`${x.pid}:${x.model}`, x.cost]));
        const nameById = new Map(db.prepare('SELECT id, name FROM targets').all().map(t => [t.id, t.name]));
        let totalCostUsd = 0;
        let totalTokens = 0;
        const kindOf = (pid) => pid === 'agent' ? 'agent' : pid === 'pipeline' ? 'pipeline' : 'model';
        ...
        const recent = db.prepare(`SELECT id, title, kind, status, saved, created_at, models,
       ...
       FROM runs ORDER BY created_at DESC LIMIT 8`).all();
        const recentRuns = recent.map(r => ({
            id: r.id, title: r.title, kind: r.kind ?? 'chat', status: r.status, saved: r.saved === 1,
            createdAt: r.created_at, participantCount: JSON.parse(r.models).length,
        ...
        const byDay = new Map(db.prepare(`SELECT strftime('%Y-%m-%d', created_at/1000, 'unixepoch') AS day, COUNT(*) AS runs
       FROM runs WHERE created_at >= ? GROUP BY day`).all(since).map(x => [x.day, x.runs]));
        const activity = [];
        for (let i = 13; i >= 0; i--) {
