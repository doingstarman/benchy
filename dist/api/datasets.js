...
function isLabeled(item, schema, type) {
    if (type === 'code')
        return (item.tests ?? '').trim() !== '';
...
export async function registerDatasetsRoutes(app) {
    ...
    app.post('/api/datasets', async (req, reply) => {
        ...
        const id = randomUUID();
        const now = Date.now();
        getDb().prepare('INSERT INTO datasets (id, name, note, type, language, schema, trusted_model, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, name, req.body.note?.trim() || null, type, language, JSON.stringify(schema), null, now, now);
    ...
    app.post('/api/datasets/:id/items', async (req, reply) => {
        ...
        if (dataset.type !== 'files' && attachmentId != null)
            return reply.code(400).send({ error: 'an input-based dataset item takes input, not a file' });
        if (dataset.type === 'files' && input)
        ...
        if (dataset.type !== 'code' && tests != null)
            return reply.code(400).send({ error: 'only a code dataset item takes tests' });
        ...
        const nextIdx = db.prepare('SELECT COALESCE(MAX(idx), -1) + 1 AS n FROM dataset_items WHERE dataset_id = ?')
            .get(req.params.id).n;
        const id = randomUUID();
        db.prepare('INSERT INTO dataset_items (id, dataset_id, idx, attachment_id, input, tests, ground_truth, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, req.params.id, nextIdx, attachmentId ?? null, input, tests, JSON.stringify(toGroundTruth(req.body.groundTruth)), Date.now());
        db.prepare('UPDATE datasets SET updated_at = ? WHERE id = ?').run(Date.now(), req.params.id);
    ...
    app.patch('/api/datasets/:id/items/:itemId', async (req, reply) => {
        ...
        if (dsRow && dsRow.type !== 'files' && req.body.attachmentId != null)
            return reply.code(400).send({ error: 'an input-based dataset item takes input, not a file' });
        if (dsRow?.type === 'files' && typeof req.body.input === 'string' && req.body.input)
            return reply.code(400).send({ error: 'a files dataset item takes a file, not input' });
        if (dsRow && dsRow.type !== 'code' && typeof req.body.tests === 'string' && req.body.tests)
            return reply.code(400).send({ error: 'only a code dataset item takes tests' });
    ...
    app.post('/api/datasets/:id/import-csv', async (req, reply) => {
        ...
        if (dataset.type === 'files' || dataset.type === 'code')
            return reply.code(400).send({ error: 'CSV import is only for text/tools datasets' });
    ...
    app.get('/api/datasets/:id/runs', async (req, reply) => {
        ...
        if (!getDatasetRow(req.params.id))
            return reply.code(404).send({ error: 'Dataset not found' });
        const rows = db.prepare(`SELECT r.id, r.status, r.models, r.total_calls, r.completed_calls, r.created_at, r.mode,
    ...
    app.post('/api/datasets/:id/run', async (req, reply) => {
        ...
        const codeLanguage = dataset.language === 'javascript' ? 'javascript' : 'python';
        if (isCode) {
        ...
        if (mode === 'arena') {
            ...
            const scored = Promise.allSettled(tasks).then(async () => {
                try {
                    await scoreCodeRun(runId, codeLanguage, items);
                }
    ...
    function nextUnjudged(verdicts, itemCount) {
    ...
    function loadArenaRun(datasetId, runId) {
        const run = getDb().prepare('SELECT models, prompts, dataset_id, mode FROM runs WHERE id = ?').get(runId);
        if (!run || run.dataset_id !== datasetId || run.mode !== 'arena')
            return null;
        ...
        const itemCount = JSON.parse(run.prompts).length;
        return { models: JSON.parse(run.models), itemCount };
    }
    app.get('/api/datasets/:id/runs/:runId/arena', async (req, reply) => {
        const arena = loadArenaRun(req.params.id, req.params.runId);
        if (!arena)
            return reply.code(404).send({ error: 'Arena run not found' });
    ...
    app.put('/api/datasets/:id/runs/:runId/verdicts/:promptIndex', async (req, reply) => {
        const arena = loadArenaRun(req.params.id, req.params.runId);
        if (!arena)
            return reply.code(404).send({ error: 'Arena run not found' });
        ...
        if (!skipped) {
            if (!best)
                return reply.code(400).send({ error: 'bestModel is required unless the item is skipped' });
            if (!arena.models.includes(best))
                return reply.code(400).send({ error: 'bestModel is not one of the run\'s models' });
            if (worst && !arena.models.includes(worst))
                return reply.code(400).send({ error: 'worstModel is not one of the run\'s models' });
            if (worst && worst === best)
                return reply.code(400).send({ error: 'bestModel and worstModel cannot be the same' });
        }
        getDb().prepare(`INSERT INTO dataset_run_verdicts (run_id, prompt_index, best_model, worst_model, skipped, created_at)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(run_id, prompt_index) DO UPDATE SET best_model = excluded.best_model, worst_model = excluded.worst_model, skipped = excluded.skipped`).run(req.params.runId, promptIndex, best, worst, skipped ? 1 : 0, Date.now());
        const verdicts = loadVerdicts(req.params.runId);
        return {
            data: {
                standings: computeStandings(arena.models, verdicts),
