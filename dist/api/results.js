    ...
    try {
        ...
        return parsed.flatMap(v => {
            if (!v || typeof v !== 'object')
                return [];
            const o = v;
            return typeof o.key === 'string' && VAR_TYPES.includes(o.type)
...
export function scoreMatrix(schema, results) {
...
function loadResults(runId) {
    return getDb().prepare('SELECT model, prompt_index, score, score_detail, total_time, input_tokens, output_tokens FROM results WHERE run_id = ? ORDER BY prompt_index, model').all(runId);
}
...
function computeWinner(runId, models, mode, results) {
    if (mode === 'arena') {
        ...
        if (!verdicts.some(v => !v.skipped && (v.bestModel || v.worstModel)))
            return null;
...
// ─── routes ──────────────────────────────────────────────────────────────────
export async function registerResultsRoutes(app) {
    const db = () => getDb();
    // The run database: every dataset test across every dataset, newest first.
    app.get('/api/results', async () => {
        const rows = db().prepare(`SELECT r.id, r.dataset_id, d.name AS dataset_name, r.prompts, r.models, r.mode, r.status, r.created_at,
        ...
        return {
            data: rows.map(r => {
                ...
                return {
                    ...
                    itemCount: JSON.parse(r.prompts).length, modelCount: models.length,
    ...
    app.get('/api/results/:runId', async (req, reply) => {
        const run = db().prepare(`SELECT r.id, r.dataset_id, d.name AS dataset_name, r.prompts, r.models, r.mode
       FROM runs r JOIN datasets d ON d.id = r.dataset_id WHERE r.id = ?`).get(req.params.runId);
        if (!run)
            return reply.code(404).send({ error: 'Not a dataset test run' });
        ...
        const schema = parseSchema(db().prepare('SELECT schema FROM datasets WHERE id = ?').get(run.dataset_id).schema);
    ...
    app.get('/api/results/:runId/export', async (req, reply) => {
        const run = db().prepare(`SELECT r.dataset_id, d.name AS dataset_name, r.mode FROM runs r JOIN datasets d ON d.id = r.dataset_id WHERE r.id = ?`).get(req.params.runId);
        if (!run)
            return reply.code(404).send({ error: 'Not a dataset test run' });
        // Export is the one path that needs the answer body, so it queries text here
        // (loadResults deliberately omits it).
        const results = db().prepare('SELECT model, prompt_index, text, score, total_time, input_tokens, output_tokens FROM results WHERE run_id = ? ORDER BY prompt_index, model').all(req.params.runId);
...
function itemFileNames(items) {
    const ids = items.map(i => i.attachment_id).filter((x) => !!x);
    const names = new Map();
    if (ids.length) {
        const rows = getDb().prepare(`SELECT id, name FROM attachments WHERE id IN (${ids.map(() => '?').join(',')})`).all(...ids);
        for (const r of rows)
