    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ flex: 1, fontSize: 'var(--fs-xs)', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
          {segment.lang}{segment.open ? ' · streaming…' : ''}
        </span>
        <IconButton onClick={copyCode} title="Copy code">
          {copied ? <IconCheck /> : <IconCopy />}
        </IconButton>
        {runnable && !running && (
          <IconButton onClick={() => setRunning(true)} title="Run"><IconPlay /></IconButton>
        )}
        {running && (
          <>
            <IconButton onClick={() => setNonce(n => n + 1)} title="Restart"><IconRefresh /></IconButton>
            <IconButton onClick={() => setRunning(false)} title="Show code" active><IconText /></IconButton>
          </>
        )}
      </div>
      {running ? (
